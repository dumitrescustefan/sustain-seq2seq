...
class LSTMDecoderWithAttentionAndPointerGenerator(LSTMDecoder):
    ...
    def forward(self, x_tuple, y_tuple, enc_output, dec_states, teacher_forcing_ratio):
        ...
        for i in range(0, seq_len_dec-1):
            ...
            # context_vector is [batch_size, encoder_size], attention_weights is [batch_size, src_seq_len, 1]
            context_vector, step_attention_weights = self.attention(state_h=dec_states[0], enc_output=enc_output, mask=src_masks)
            ...
            vocab_dist = torch.softmax(self.vocab_linear(torch.tanh(lin_output)).squeeze(1), dim=1)
            ...
            p_gen = self.context_linear(context_vector) + \
                ...
                self.p_gen_bias
            p_gen = self.p_gen_linear(torch.sigmoid(p_gen))
            
            # Calculate final distribution, final_dist will be [batch_size, n_class]
            # vocab_dist is [batch_size, n_class], attention_weights is [batch_size, src_seq_len, 1], src is [batch_size, src_seq_len] and contains indices
            # first, we must use attention_weights to get attention_dist to be [batch_size, n_class]
            attention_dist = torch.zeros(batch_size, self.n_class).to(self.device)
            for instance_index in range(batch_size):
                for word_index in range(src_seq_len):
                    attention_dist[instance_index][src[instance_index][word_index]] = attention_weights[instance_index][word_index]
