...
def prepare_dataloaders(data_folder, batch_size, max_seq_len = 100000, min_seq_len = 1):
    train_loader = torch.utils.data.DataLoader(
        CNNDMDataset(data_folder, "train", max_seq_len, min_seq_len),
        num_workers=1,
    ...
    valid_loader = torch.utils.data.DataLoader(
         CNNDMDataset(data_folder, "dev", max_seq_len, min_seq_len),
        num_workers=1,
        batch_size=batch_size,
        collate_fn=simple_paired_collate_fn)
    
    test_loader = torch.utils.data.DataLoader(
         CNNDMDataset(data_folder, "test", max_seq_len, min_seq_len),
        num_workers=1,
        batch_size=batch_size,
        collate_fn=simple_paired_collate_fn)
...
def prepare_dataloaders_with_pos(data_folder, batch_size, max_seq_len, min_seq_len):
    train_loader = torch.utils.data.DataLoader(
        CNNDMDataset(data_folder, "train", max_seq_len, min_seq_len),
        num_workers=1,
        batch_size=batch_size,
