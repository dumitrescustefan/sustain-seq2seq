# -*- coding: utf-8 -*-
"""
Created on Tue Dec 18 10:11:11 2018

@author: andrei.apostol
"""
...
class Lang:
    def __init__(self, name):
        self.name = name
        #indexes every word from 2 to n (0 and 1 belongs to SOS and EOS)
        self.word2index = {}
        
        #counts the number of apperences for each word
        self.word2count = {}
...
def readLangs(lang1, lang2, reverse=False):
    ...
    lines = open(r'E:\LST with attention\data\%s-%s.txt' % (lang1, lang2), encoding='utf-8').\
        read().strip().split('\n')
...
class EncoderRNN(nn.Module):
    ...
    def initHidden(self):
        w = torch.empty(1, 1, self.hidden_size, device=device)
...
def trainIters(encoder, decoder, n_iters, print_every=1000, plot_every=100, learning_rate, use_attention):
    start = time.time()
    plot_losses = []
...
decoder = DecoderRNN(input_lang.n_words, args['hidden_size']).to(device)
att_decoder = AttnDecoderRNN(output_lang.n_words, args['hidden_size'], args['dropout_p']).to(device)    
